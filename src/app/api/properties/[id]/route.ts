...
const ALLOWED_BRANDING_FIELDS = ['logo', 'primaryColor', 'secondaryColor', 'fontFamily', 'tagline', 'contactEmail', 'contactPhone', 'address', 'socialLinks', 'heroImage'];
...
function sanitizeSettings(body: Record<string, unknown>) {
  const allowed = ['timezone', 'currency', 'unitSystem', 'bookingLeadDays', 'cancellationPolicy', 'checkInTime', 'checkOutTime', 'minNightStay', 'maxNightStay', 'instantBooking', 'taxRate'];
...
export async function PATCH(
  ...
  try {
    ...
    if (body.subdomain !== undefined) {
      const cleanSub = String(body.subdomain).trim().toLowerCase().replace(/[^a-z0-9-]/g, '');
      if (body.subdomain !== null && !cleanSub) {
        return NextResponse.json({ error: 'Invalid subdomain' }, { status: 400 });
      }
      if (cleanSub) {
        const existing = (await db.prepare('SELECT id FROM properties WHERE subdomain = ? AND id != ?').get(cleanSub, id)) as any;
        if (existing) {
    ...
    if (branding) {
      const current = (await db.prepare('SELECT branding FROM properties WHERE id = ?').get(id)) as any;
      let existingBranding: Record<string, unknown> = {};
      if (current?.branding) {
        existingBranding = typeof current.branding === 'string' ? JSON.parse(current.branding) : current.branding;
      }
    ...
    if (settings) {
      const current = (await db.prepare('SELECT settings FROM properties WHERE id = ?').get(id)) as any;
