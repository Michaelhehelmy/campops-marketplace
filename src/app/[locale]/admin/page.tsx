  ...
  if (loading) {
    return (
      <div className="p-8 max-w-7xl mx-auto space-y-8 animate-pulse">
        <div className="h-10 w-64 bg-slate-900/10 rounded-xl mb-4"></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-slate-950 p-8 rounded-[2.5rem] border border-slate-900 shadow-2xl relative overflow-hidden">
          ...
          <div className="h-64 flex items-end justify-between gap-3 px-4">
            {[45, 55, 62, 50, 78, 85, 92, 110, 105, 120, 125, 140].map((h, i) => (
              <div key={i} className="group relative flex-1">
                <div
                  style={{ height: `${h}%` }}
                  className="bg-gradient-to-t from-amber-500 to-yellow-400 rounded-t-lg opacity-80 group-hover:opacity-100 transition-all cursor-pointer shadow-[0_0_10px_rgba(245,158,11,0.2)]"
                ></div>
...
function StatCard({ title, value, icon: Icon, color }: any) {
  ...
  return (
    <div className="bg-slate-950 p-6 rounded-[2rem] shadow-2xl border border-slate-900 hover:border-slate-850 transition-all flex flex-col justify-between">
