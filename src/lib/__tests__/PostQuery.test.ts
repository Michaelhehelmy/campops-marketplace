...
describe('PostQuery', () => {
  ...
  it('returns posts filtered by postType', async () => {
    await repo.createPost({ siteId: 's1', postType: 'listing', postTitle: 'Tent A' });
    await repo.createPost({ siteId: 's1', postType: 'booking', postTitle: 'Booking 1' });
  ...
  it('accepts array of postTypes', async () => {
    await repo.createPost({ siteId: 's1', postType: 'listing', postTitle: 'L' });
    await repo.createPost({ siteId: 's1', postType: 'room', postTitle: 'R' });
    await repo.createPost({ siteId: 's1', postType: 'booking', postTitle: 'B' });
  ...
  it('supports search on post_title', async () => {
    await repo.createPost({ siteId: 's1', postType: 'listing', postTitle: 'Riverside Tent' });
    await repo.createPost({ siteId: 's1', postType: 'listing', postTitle: 'Mountain Lodge' });
  ...
  it('respects limit and offset', async () => {
    for (let i = 0; i < 5; i++) {
      await repo.createPost({ siteId: 's1', postType: 'listing', postTitle: `Post ${i}` });
    }
  ...
  it('skips meta loading when includeMeta=false', async () => {
    await repo.createPost({ siteId: 's1', postType: 'listing', postTitle: 'A', meta: { k: 'v' } });
  ...
  it('count returns correct total', async () => {
    ...
    await repo.createPost({ siteId: 's1', postType: 'listing', postTitle: 'B' });
  ...
  it('scopes results to site only', async () => {
    db.prepare("INSERT INTO sites (id, slug, name) VALUES ('s2', 'other', 'Other')").run();
    await repo.createPost({ siteId: 's1', postType: 'listing', postTitle: 'Mine' });
    await repo.createPost({ siteId: 's2', postType: 'listing', postTitle: 'Theirs' });
